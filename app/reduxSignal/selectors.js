...
export const getModal = customSelectorCreator(
  ...
  instanceId =>
    createSelector(
      state => state.get('signal').get('modals'),
      modals => {
...
export const getSignal = createSelector(
  state => state.get('signal').get('signal'),
  //state => state.get('signal').get('signal'),
...
export const getModalEvents = createSelector(
  ...
  eventQueueId =>
    createSelector(
      state => state.get('signal').getIn(['signal', 'eventQueue']),
...
export const getModalByEventQueueId = createSelector(
  ...
  eventQueueId =>
    createSelector(
      state => state.get('signal').getIn(['signal', 'data']),
      data => {
        if (!data) {
          return null
        }
